...
# Features = ['Data Mean', 'Data Std', 'Data Spread', 'SB_BinaryStats', 'Delta Mean',
#             'Delta Std', 'Delta Spread', 'MD_hrv', 'Repeat', 'Non-C Repeat', 'Increase']
...
def LR(k, position=None):
    ...
    if position != None:
        plot_confusion_matrix(
            ...
            save="LR_{}.eps".format(position),
        )
    else:
        logger.write(
            "{},{},{},{}\n".format("LR", print_metrices(pred, y_test), end - start, k)
        )


def SVM(k, position=None):
    start = time.time()
    ...
    if position != None:
        plot_confusion_matrix(
            ...
            save="SVM_{}.eps".format(position),
        )
    else:
        logger.write(
            "{},{},{},{}\n".format("SVM", print_metrices(pred, y_test), end - start, k)
        )


def DT(k, position=None):
    start = time.time()
    ...
    if position != None:
        plot_confusion_matrix(
            ...
            save="DT_{}.eps".format(position),
        )
    else:
        logger.write(
            "{},{},{},{}\n".format("DT", print_metrices(pred, y_test), end - start, k)
        )


def RF(k, position=None):
    start = time.time()
    pipeline = Pipeline(
    ...
    if position != None:
        plot_confusion_matrix(
            ...
            save="RF_{}.eps".format(position),
        )
    else:
        logger.write(
            "{},{},{},{}\n".format("RF", print_metrices(pred, y_test), end - start, k)
        )


def GDBT(k, position=None):
    start = time.time()
    pipeline = Pipeline(
    ...
    if position != None:
        plot_confusion_matrix(
            ...
            save="GDBT_{}.eps".format(position),
        )
    else:
        logger.write(
            "{},{},{},{}\n".format("GDBT", print_metrices(pred, y_test), end - start, k)
        )


def MLP(k, position=None):
    start = time.time()
    pipeline = Pipeline(
        [
            (
    ...
    if position != None:
        plot_confusion_matrix(
            ...
            save="MLP_{}.eps".format(position),
        )
    else:
        logger.write(
            "{},{},{},{}\n".format("MLP", print_metrices(pred, y_test), end - start, k)
        )


RESULT_PATH = "./train_result.csv"

logger = open(RESULT_PATH, "w")
logger.write("Model,Precison,Recall,F1,Time Cost,k\n")
...
# prepare("./train_Real.csv")
# DT("real-world")
# RF("real-world")

# prepare("./train_Synthetic.csv")
# DT("synthetic")
# RF("synthetic")
