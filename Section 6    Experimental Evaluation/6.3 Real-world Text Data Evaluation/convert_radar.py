...
INPUT_PATH = "../../Real_Text_result.csv"
...
for Compressor in Compressors:
    ...
    ef = ef[['Algorithm', "Encoding Time", "Decoding Time",
             "Compress Time", "Uncompress Time", "Compression Ratio"]]
    ef['order'] = pd.Categorical(
        ef['Algorithm'], categories=Encoders, ordered=True)
    ef = ef.sort_values('order').drop('order', axis=1)
    ef.to_csv('{}_avg.csv'.format(Compressor.lower()), index=False)
